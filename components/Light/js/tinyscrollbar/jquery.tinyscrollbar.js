...
(function ($) {
  ...
  function Scrollbar(root, options) {
    ...
    this.update = function (sScroll) {
      ...
      oTrack[options.axis] =
        options.size == "auto" ? oViewport[options.axis] : options.size;
      oDragger[options.axis] = Math.min(
        oTrack[options.axis],
        Math.max(
          0,
          options.draggerWidth == "auto"
            ? oTrack[options.axis] * oImages.ratio
            : options.draggerWidth
        )
      );
      oScrollbar.ratio =
        options.draggerWidth == "auto"
          ? oImages[options.axis] / oTrack[options.axis]
          : (oImages[options.axis] - oViewport[options.axis]) /
            (oTrack[options.axis] - oDragger[options.axis]);
      iScroll =
        sScroll == "relative" && oImages.ratio <= 1
          ? Math.min(
              oImages[options.axis] - oViewport[options.axis],
              Math.max(0, iScroll)
            )
          : 0;
      iScroll =
        sScroll == "bottom" && oImages.ratio <= 1
          ? oImages[options.axis] - oViewport[options.axis]
      ...
      oScrollbar2.ratio =
        options.draggerWidth == "auto"
          ...
          : (oContent[options.axis] - oMilestones[options.axis]) /
            (oTrack[options.axis] - oDragger[options.axis]);
      iScroll2 =
    ...
    function drag(oEvent) {
      if (!(oImages.ratio >= 1)) {
        ...
        $(oImages.obj)
          .stop(true, true)
          .animate({ left: -iScroll }, 1000, "easeInOutQuad");
        $(oContent.obj)
          .stop(true, true)
          .animate({ left: -iScroll2 }, 700, "easeInOutQuad");
        $(oDragger.obj)
          .stop(true, true)
          .animate({ left: iPosition.now }, 700, "easeInOutQuad");
